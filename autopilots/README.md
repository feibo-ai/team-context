...
- **autopilot = 调度载体**:每 kind 一个 `<kind>-<suffix>`,description 只含该任务的
  差异 prompt(YAML 的 desc + prompt),`--agent` 指向本 scope 的 assistant-bot。
  cron/observability/guardrails 按任务独立保留。

AUTOPILOT_SCOPE 是 agent 级单值 ⇒ 个人与 team **不能**共用 agent(DRI 机器上是
`assistant-bot-actionow.ai` + `assistant-bot-team` 两个)。

实况分布(2026-06-10 迁移时,不补历史缺口):bairongcan 2 / fys 5 / qiuhaoqi 5 /
xieyu 5 / actionow.ai 个人 4(无个人 daily-kickoff)/ team 1(仅 daily-kickoff)= 22 个 autopilot。
...
(TEA-93 起 `agent.name` 字段已删 —— 身份 agent 名由脚本派生为 `assistant-bot-<suffix>`。)
...
| 看终态对象 | `multica agent list \| grep assistant-bot`(6 个)· `multica agent list --include-archived \| grep -- -bot-`(legacy 已归档) |
