...
# Claude: ~/.claude/skills · Codex: ~/.agents/skills (原生 agentskills.io 发现,无需 index)
bash scripts/sync-team-config.sh
```

完事。重启 Claude Code · 试 "接手 issue" · 应该弹 tc-router skill(试 "我想 /clear" → tc-handoff)。
...
# 同步 13 个 skill 到 multica workspace
...
│
├── skill-pack.yaml                    ← pack manifest (版本 · owner · review 日期 · 不随 skill 下发)
├── evals/                             ← 包级测试 (validate-skills.py CI lint · render pytest · 冒烟清单)
│
├── skills/                            ← 13 个标准 skill (agentskills.io 格式 · 见 skills/README.md)
│   ├── tc-router/          ← 人类唯一推荐入口 · 接手/继续/卡住了 → 判断下一步
│   ├── tc-kickoff/         ← Phase 01 6 步 wizard (原 tc-1-start)
│   ├── tc-research/        ← Research session 协议 (原 tc-2-research)
│   ├── tc-plan/            ← Plan session · 4 必备字段 + 角色分工 (吸收 tc-roles)
│   ├── tc-design-review/   ← 设计评审 gate
│   ├── tc-build/           ← Implement 纪律 (原 tc-4-build)
│   ├── tc-health/          ← 污染 4 信号扫描 + 10 反 pattern 自检 (合并 tc-health-check/tc-self-check)
│   ├── tc-handoff/         ← /clear 前检查
│   ├── tc-review/          ← case file 5 section debrief (原 tc-5-review)
│   ├── tc-conflict/        ← 冲突 4 原则 → decisions/
│   ├── tc-rhythm/          ← 周一对齐 + 周五 demo/betting (合并 tc-monday/tc-friday)
│   ├── tc-ops/             ← 运维脚本 (monthly_health · issue_invariants · autopilot_lint)
│   └── tc-render/          ← 工具型基座: publish.py + transition.py + 单一真相 references
│
...
│   ├── labels.md · multica-fields.md  ← 指针存根 (正文已迁 tc-render/references/ · 随 skill 分发)
...
│
└── .github/workflows/
    └── lint.yml                       ← evals/validate-skills.py (skill pack) + autopilot YAML schema
```
...
claude_md_team_global.md           ← L1 抽象层 · 每个 session 自动加载 · 硬上限 3k token
   ↓ 调用
13 个 skill (body ≤1500 字符 each) ← 触发式 · tc-router 兜底路由 · 按场景激活
```
...
L3 中间 (本仓库)
  skills/  ← 被验证 prompt · body ≤1500 字符 each · Agent 自动触发
...
- skills/* 谁 own / 90 天未 review? skill-pack.yaml + `multica skill list --stale` + tc-ops/scripts/monthly_health.py 列出
- claude_md_team_global.md token 数过 2800? CI 失败 → 手动 prune

每季度（3/6/9/12 月）额外：
- CLAUDE.md prune 强制（过 3.5k 必须删）
- skills/ 没人调用的归档
- tc-render/references/issue-label-state-rules.md vs multica workspace 实际 label 差异 diff

## 加新东西怎么走

| 加什么 | 怎么走 |
|---|---|
| 新 skill (跨项目) | PR · 标准目录 (SKILL.md 只带 name/description) + skill-pack.yaml 登记 owner/last_reviewed_at + evals/validate-skills.py pass |
| 新 autopilot YAML | PR · 含完整 guardrails 段 (tc-ops/autopilot_lint.py 必须 pass) |
| 新 label | 改 `skills/tc-render/references/issue-label-state-rules.md` + 重跑 `create-labels.sh` · PR review |
...
| 调 CLAUDE.md (L1 提升) | 经 tc-review 流程手动追加(原 case_promote_rule)· 走月度 review |
