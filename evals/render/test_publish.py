...
跑:  python3 -m pytest evals/render/ -q
"""
...
REPO_ROOT = Path(__file__).resolve().parents[2]              # team-context/
SKILL_DIR = REPO_ROOT / "skills" / "tc-render"
SKILLS_ROOT = SKILL_DIR.parent                               # skills/
PUBLISH_PY = SKILL_DIR / "scripts" / "publish.py"
...
DOC_SKILLS = ["tc-research", "tc-plan", "tc-review", "tc-handoff"]
...
def test_tc_render_skill_describes_gate_b_not_a():
    """②文档无污染:tc-render/SKILL.md 的发布路径必须是 publish 脚本收口(CLI --inline),
    不得把 raw-HTTP 灾备当主路径(防 routing 层文档漂移误导 agent 手拼 HTTP 两步)。"""
    txt = (SKILL_DIR / "SKILL.md").read_text()
    assert "publish.py" in txt, "tc-render/SKILL.md 应把 publish 脚本作为发布路径"
    assert "curl" not in txt and "/api/" not in txt, "tc-render/SKILL.md 不得描述 raw-HTTP 发布路径"
    contract = (SKILL_DIR / "references" / "publish-contract.md").read_text()
    assert "唯一路径" in contract and "--inline" in contract, "契约文档应明确 CLI --inline 收口为唯一发布路径"
    assert "灾备" in contract, "契约文档应把 raw-HTTP 标注为灾备而非主路径"


def test_contract_doc_no_raw_curl_bypass():
    """①:publish-contract.md 的散文部分不得含可执行的 raw-HTTP runbook
    (绕过 publish.py 硬校验的旁路);机器可读灾备契约的代码块本体豁免。

    查的是**可执行的 curl 命令签名**,不是禁词 —— 散文里『不要手拼 curl』这类禁令措辞允许。
    """
    txt = (SKILL_DIR / "references" / "publish-contract.md").read_text()
    prose = re.sub(r"```.*?```", "", txt, flags=re.S)   # 剥离代码块(灾备契约/用法示例)
    assert "curl -" not in prose, "publish-contract.md 散文仍含可执行 curl 命令(手跑 raw-HTTP 旁路)"
    assert "/api/upload-file" not in prose, "publish-contract.md 散文仍含裸 upload 端点 runbook(旁路)"
