...
# 5. feishu-cli (team-context 仓库一键脚本)
...
bash scripts/install-feishu-cli.sh      # 引导 feishu config + 拿 chat_id

cd ../team-context-mcp
pnpm install && pnpm build              # 出 dist/server.js
```
...
重启 Claude Code。然后：
```bash
codex mcp add team-context-mcp -- node ~/team-context-mcp/dist/server.js
```
...
| Claude Code 新 session 问 "What tools do you have from team-context-mcp?" | 列出 **17** 个 |
...
Step 6 · Broadcast
   调 MCP `project_kickoff` 自动飞书广播 (含 step 1-5 全部信息)
...
| Day 1 上午 | 旁站着帮新人过 setup · 5 工具 doctor · 任一不过立刻一起 debug |
