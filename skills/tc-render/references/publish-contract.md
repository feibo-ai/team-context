...
python3 "$TCR/scripts/publish.py" --type {plan|research|case|handoff} \
  --data fields.json --issue <完整UUID或issue-key> [--caption "标题(下方渲染)"] [--out 本地落盘路径.html] [--dry-run] [--no-transition]
```
...
- `--issue` 接受**完整 UUID 或 issue key**(如 TEA-88);非 UUID 输入由脚本经 `multica issue show` 解析为完整 UUID 再进 API(进 API 一律完整 UUID 的命门不变,解析失败即硬失败)。
