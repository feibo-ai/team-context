...
- **case**:`goal`◆ · `whatHappened`◆ · `slug` · `criteriaResults`[{`criterion`,`met`(bool),`notMetReason`?}](结论格三态:「已收尾 · 标准全数达成」/「已收尾 · n/m 达成」/缺省「复盘存档」) · `keyJudgments`[{`title`,`context`,`options`[],`chose`,`inHindsight`,`ancientImpossible`}](≥1 · section4 折叠空白后 ≥100 字;title 非空项进首屏「要点提示」) · `ruleCandidates`[]?
