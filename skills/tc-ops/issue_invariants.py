...
6 条硬性不变量(适用于带流程 label 的 issue;零 label 轻任务不在审计内):
  1. 复盘-已审   ⇒ status done
  2. 复盘-待审   ⇒ status in_review
  3. 计划-评审中 ⇒ status in_review
  4. 计划-已批准 ⇒ status ∈ {todo, in_progress, done}
  5. cancelled  ⇒ 无任何流程 label
  6. 计划-已升级 ⊕ 计划-已批准 互斥

3 档警告(不算违规,提示人看):
  - staleness:计划-评审中 / 复盘-待审 且 issue updated_at 超 48h(以 updated_at 近似
    label 挂上时间——编排 session 在 verdict 后死亡的残留态信号)
...
def check(issues, now=None):
    ...
    for it in issues:
        ...
        if plabels:
            ...
            if "计划-已批准" in labels and status not in ("todo", "in_progress", "done"):
            ...
            if labels & {"计划-评审中", "复盘-待审"} and status != "cancelled":
                ...
                try:
                    ...
                    if hours > STALE_HOURS:
                        warnings.append((ident, "评审挂起 %.0fh(>%dh):%s — verdict 后编排 session 死亡?"
                                         % (hours, STALE_HOURS, "、".join(sorted(labels & {"计划-评审中", "复盘-待审"})))))
