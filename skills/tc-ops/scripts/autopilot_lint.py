#!/usr/bin/env python3
"""autopilot lint — 校验 autopilot YAML 是否满足 PB-04 guardrails。

纯本地确定性校验,无需 MCP 服务器。有 error 则 exit 1。
用法:  autopilot_lint.py <yaml-path> [<yaml-path> ...]

agent.name 不是 YAML 字段(身份 agent assistant-bot-<scope> 由 team-context 仓库的
autopilot 公共 shell 库 _autopilot-common.sh 派生);
本脚本校验同目录 _agent-instructions.md(通用约束单源)存在且非空。
三处 lint 同步(改规则同时改):team-context 仓库 CI workflow lint / 这里 /
_autopilot-common.sh 的 ac_lint_yaml。
改本文件必须同步 multica registry(multica skill files upsert tc-ops),否则 monthly-health
autopilot 的 `multica skill pull tc-ops` 会经本地 skills 软链把 registry 旧版写回 repo 工作树。
"""
import os
import sys
import yaml
